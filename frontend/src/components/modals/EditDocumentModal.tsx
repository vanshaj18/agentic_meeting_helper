...
const EditDocumentModal: React.FC<EditDocumentModalProps> = ({ document, onClose, onUpdate }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="p-4 md:p-5 border-b border-gray-200 flex items-center justify-between sticky top-0 bg-white z-10">
        ...
        <div className="p-4 md:p-5 space-y-3 md:space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-900 mb-1.5">Document Title</label>
            <input
              type="text"
        ...
        <div className="p-4 md:p-5 border-t border-gray-200 flex flex-col sm:flex-row gap-3 sticky bottom-0 bg-white">
          <button
            onClick={onClose}
            className="w-full sm:flex-1 px-4 md:px-6 py-2 md:py-2.5 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors text-sm md:text-base"
          >
            Cancel
          </button>
          <button
            onClick={handleSubmit}
            className="w-full sm:flex-1 px-4 md:px-6 py-2 md:py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium text-sm md:text-base"
          >
            Update Document
          </button>
        </div>
